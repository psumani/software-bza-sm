...
namespace ZiveLab.ZM.ZIM
{
    public class CommObj : IDisposable
    {
        ...
        public bool WriteData(int nSlot, stSystemConfig data) /* sang */
        {
            data.mZimCfg[0].ranges.Gen.ID = DeviceConstants.ID_RANGEINFO;
            if (isConnected == false) return false;
            if (mComm.WriteToDevice(CommandSet.SET_MCBZA_RNGINFO, nSlot, data.ToByteArray()) == false)
            {
                mComm.Dispose();
                return false;
            }
        ...
        {
            data.Gen.ID = DeviceConstants.ID_RANGEINFO;
            if (isConnected == false) return false;
        ...
        public bool ReadData(int nSlot, ref stZimCfg data) // bd Data
        {
            byte[] buf;
            if (isConnected == false) return false;
        ...
        public bool WriteSIFPoint(int nSlot, stSIFCfg data)
        {
            if (!isConnected) return false;

            byte[] buf = data.ToByteArray();

            if (!mComm.WriteToDevice(CommandSet.SET_SIF_POINT, nSlot, buf))
            {
                mComm.Dispose();
                return false;
            }
        ...
        public bool ReadSIFPoint(int nSlot, stSIFCfg data)
        {
            byte[] buf;
            if (isConnected == false) return false;
            buf = mComm.ReadFromDevice(CommandSet.SET_SIF_POINT, nSlot);
        ...
        public bool ReadData(ref stDevInf data)
        {
            ...
            if (Sifid == DeviceConstants.ID_ZIMCONFIG_1)
            {
                ...
                { 
                    ...
                    data.mSysCfg.mZimCfg[bd].ranges.Gen.ToWritePtr(m_devinf_1.mSysCfg.mZimCfg[bd].ranges.ToByteArray()); // 배열
                    for (int auxch = 1; auxch < MBZA_Constant.MAX_AUX_CHANNEL; auxch++)
                    {
        ...
        public bool ReadRangeInfOfZim(int nSlot, int nAddr, ref st_zim_rnginf data)
        {
            ...
            data.Gen.ToWritePtr(buf);

            if (data.Gen.ID != DeviceConstants.ID_RANGEINFO)
            {
        ...
        public bool ProgConfigOfZim(int nSlot, ref stZimCfg data)
        {
            ...
            data.ranges.Gen.ID = DeviceConstants.ID_RANGEINFO; // 배열
