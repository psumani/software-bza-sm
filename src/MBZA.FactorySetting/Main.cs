...
namespace ZiveLab.ZM.FactorySetting
{
        public partial class Main : Form
    {
        ...
        public void RefreshHostname()
        {
            ...
            {
                if (mSysCfg.mSIFCfg.Type == (byte)eDeviceType.ZIM || mSysCfg.mSIFCfg.Type == (byte)eDeviceType.SBZA || mSysCfg.mSIFCfg.Type == (byte)eDeviceType.MBZA || mSysCfg.mSIFCfg.Type == (byte)eDeviceType.MCBZA)
                {
            ...
            lnklblchghostname.Text = "> Hostname:Z0000";
        }
...
//              LnklblRefSifWebInf.Enabled = false;
            ...
            lnklblchghostname.Enabled = mCommZim.isConnected;
            if(mCommZim.isConnected)  lnklblchghostname.Text = string.Format("> Hostname:{0}", Encoding.Default.GetString(mConnCfg.mEthernetCfg.hostname).Trim('\0'));
            else lnklblchghostname.Text = "> Hostname:Z0000";
        }
        ...
        public void ViewZimInfo(int bd)
        {
            if (ChkEnaCh.Checked == true)
            {
                if (mCommZim.isConnected == true)
                {
                    ...
                    {
                        if (mCommZim.CheckROMofZIM(bd, false) == true) // true
                        {
        ...
        private void BtInitZIM_Click(object sender, EventArgs e)
        {
            if (mCommZim.isConnected == true)
            {
                ...
                for (auxch = 0; auxch < MBZA_Constant.MAX_AUX_CHANNEL; auxch++)
                {
                    ...
                    mSysCfg.mZimCfg[idevbd].ranges.Initialize(type); // 배열
                }
