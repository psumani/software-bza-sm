...
namespace ZiveLab.Device.ZIM.Interface
{
    public class SIFCom
    {
        ...
        public byte[] ReadFromDevice(byte command, int address = 0)
        {
            if (!isConnected)
                return null;
            ...
            try
            {
                stCmdHeader sHeader = new stCmdHeader(command, address);
                if (!Write(sHeader.ToByteArray()))
                    return null;

                sHeader = new stCmdHeader(SIFConstants.CMD_READ_DATA);
        ...
        public byte[] ReadFromDevice(byte command, int device, int slot)
        {
            if (!isConnected)
                return null;
            ...
            try
            {
                stCmdHeader sHeader = new stCmdHeader(command, device, slot, 0, 0);
                if (!Write(sHeader.ToByteArray()))
                    return null;
        ...
        public bool WriteToDevice(byte command)
        {
            ...
            try
            {
                stCmdHeader sHeader = new stCmdHeader(command);
                if (!Write(sHeader.ToByteArray()))
                    return false;
        ...
        public bool WriteToDevice(byte command, int address, byte[] buffer)
        {
            bool result = false;
            ...
            try
            {
                ...
                stCmdHeader sHeader = new stCmdHeader(command, address, bLen);
        ...
        public bool WriteToDevice(byte command, int device, int slot, byte[] buffer)
        {
            bool result = false;
            ...
            {
                int bLen = (buffer == null) ? 0 : buffer.Length;
                stCmdHeader sHeader = new stCmdHeader(command, device, slot, 0, bLen);
                int headersize = Marshal.SizeOf(typeof(stCmdHeader));
                int size = headersize + bLen;
        ...
        public bool DC_EraseBlockNand(object data)
        {
            ...
            try
            {
                ...
                result = WriteToDevice((byte)FirmwareCommands.NandBlockErase, block, null);
            }
            catch (Exception)
            {
                return false;
        ...
        public bool DC_ProgramNand(object data)
        {
            ...
            try
            {
                ...
                result = WriteToDevice((byte)FirmwareCommands.NandProgram, page, buffer);
            }
            catch (Exception)
            {
                return false;
        ...
        public bool DC_ProgramNand(int block, byte[] buffer)
        {
            ...
            try
            {
                result = WriteToDevice((byte)FirmwareCommands.NandProgram, block, buffer);
            }
            catch (Exception)
            {
                return false;
        ...
        public bool DC_SetFirmwareInfo(object data)
        {
            ...
            try
            {
                ...
                result = WriteToDevice((byte)FirmwareCommands.SetFirmwareInfo, 0, buffer);
            }
            catch (Exception)
            {
                return false;
