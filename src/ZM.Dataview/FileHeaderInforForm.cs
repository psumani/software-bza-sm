...
namespace ZiveLab.ZM.Dataview
{
    public partial class FileHeaderInforForm : Form
    {
        ...
        private void FormFileHeaderInfor_Load(object sender, EventArgs e)
        {
            ...
            richTextBox1.AppendText(string.Format("  * {0} : {1}(v{2}):{3}\r\n", Properties.Resources.ZIM_Board, _DataHeaderValues._ResHead.sysInfo.mZimCfg[_DataHeaderValues._ResHead.mInfo.sifch].GetZimTypeString(),
                                                                  _DataHeaderValues._ResHead.sysInfo.mZimCfg[_DataHeaderValues._ResHead.mInfo.sifch].GetFirmwareVer(), _DataHeaderValues._ResHead.sysInfo.mZimCfg[_DataHeaderValues._ResHead.mInfo.sifch].GetSerialNumber()));
            ...
            if (_DataHeaderValues._ResHead.sysInfo.mSIFCfg.GetDeviceType() == eDeviceType.MCBZA)
            {
                ...
                for (int i = 1; i < MBZA_Constant.MAX_DEV_CHANNEL; i++)
                {
                    ...
                    {
                        str = string.Format("{0}(v{1}):{2}", _DataHeaderValues._ResHead.sysInfo.mZimCfg[i].GetZimTypeString(), _DataHeaderValues._ResHead.sysInfo.mZimCfg[i].GetFirmwareVer(), _DataHeaderValues._ResHead.sysInfo.mZimCfg[i].GetSerialNumber());
