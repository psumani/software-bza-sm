...
namespace ZiveLab.ZM.Dataview
{
    ...
    public class DataviewCommon
    {
        ...
        static public string[] CreateInforArray(DataHeaderValues dhv)
        {
            ...
            sSifSerial = dhv._ResHead.sysInfo.mSIFCfg.GetSerialNumber();
            sSifFwVer = dhv._ResHead.sysInfo.mSIFCfg.GetFirmwareVer();
            sSifType = string.Format("{0}({1})", dhv._ResHead.sysInfo.mSIFCfg.GetProductTypeString(), dhv._ResHead.sysInfo.mSIFCfg.GetDeviceTypeString());
            sZimSerial = dhv._ResHead.sysInfo.mZimCfg[dhv._ResHead.mInfo.sifch].GetSerialNumber();
            sZimType = dhv._ResHead.sysInfo.mZimCfg[dhv._ResHead.mInfo.sifch].GetZimTypeString();
            sZimBoardType = dhv._ResHead.sysInfo.mZimCfg[dhv._ResHead.mInfo.sifch].GetBoardTypeString();
            sZimFwVer = dhv._ResHead.sysInfo.mZimCfg[dhv._ResHead.mInfo.sifch].GetFirmwareVer();
            ...
            if (dhv._ResHead.sysInfo.mSIFCfg.GetDeviceType() == eDeviceType.MCBZA)
            {
                for (int i = 1; i < MBZA_Constant.MAX_DEV_CHANNEL; i++)
                {
                    if (dhv._ResHead.sysInfo.ChkZIM[i] == 1)
                    {
                        list.Add(string.Format("  * {0}[{1}] : {2}", Properties.Resources.AuxBoardInfo, i, string.Format("{0}(v{1})/ {2}/ SIF(v{1}):{3}-{4}", dhv._ResHead.sysInfo.mZimCfg[i].GetZimTypeString(), dhv._ResHead.sysInfo.mZimCfg[i].GetFirmwareVer(), dhv._ResHead.sysInfo.mZimCfg[i].GetSerialNumber())));
                    }
                }
            }
            list.Add("");
