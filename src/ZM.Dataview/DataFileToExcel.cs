...
namespace ZiveLab.ZM.Dataview
{
    ...
    public class UnitToExcelDataInfor
    {
        ...
        public string[] GetInforArray()
        {
            ...
            sSifSerial = _ResHead.sysInfo.mSIFCfg.GetSerialNumber();
            sSifFwVer = _ResHead.sysInfo.mSIFCfg.GetFirmwareVer();
            sSifType = string.Format("{0}({1})", _ResHead.sysInfo.mSIFCfg.GetProductTypeString(), _ResHead.sysInfo.mSIFCfg.GetDeviceTypeString());
            sZimSerial = _ResHead.sysInfo.mZimCfg[_ResHead.mInfo.sifch].GetSerialNumber();
            sZimType = _ResHead.sysInfo.mZimCfg[_ResHead.mInfo.sifch].GetZimTypeString();
            sZimBoardType = _ResHead.sysInfo.mZimCfg[_ResHead.mInfo.sifch].GetBoardTypeString();
            sZimFwVer = _ResHead.sysInfo.mZimCfg[_ResHead.mInfo.sifch].GetFirmwareVer();
            list.Add(sReportname);
            list.Add("");
            list.Add(string.Format("  * {0} : {1}", Properties.Resources.Test_Data_File, sDatafile));
            list.Add(string.Format("  * {0} : {1}", Properties.Resources.Schedule_File_Name, sSchfile));
            list.Add("");
            list.Add(string.Format("  * {0} : {1}", Properties.Resources.ChInfo, string.Format("{0}/ {2}/ SIF(v{1}):{3}-{4}", sCh, sSifFwVer, sSifType, sSifSerial, sSifCh)));
            if (_ResHead.sysInfo.mSIFCfg.GetDeviceType() == eDeviceType.MCBZA)
            {
                for (int i = 1; i < MBZA_Constant.MAX_DEV_CHANNEL; i++)
                {
                    if (_ResHead.sysInfo.ChkZIM[i] == 1)
                    {
                        list.Add(string.Format("  * {0}[{1}] : {2}", Properties.Resources.AuxBoardInfo, i, string.Format("{0}(v{1})/ {2}/ SIF(v{1}):{3}-{4}", _ResHead.sysInfo.mZimCfg[i].GetZimTypeString(), _ResHead.sysInfo.mZimCfg[i].GetFirmwareVer(), _ResHead.sysInfo.mZimCfg[i].GetSerialNumber())));
                    }
                }
            }
            list.Add("");
