...
typedef enum { DEV_UNKNOWN, DEV_BZA1000A, DEV_BZA1000, DEV_BZA500, DEV_BZA100,DEV_BZA60,DEV_BZAAUX1} eDeviceType;
typedef enum { SIF_WBCS, SIF_SMART, SIF_ZIM, SIF_SBZA, SIF_MBZA ,SIF_ZBCS,SIF_CXM,SIF_BZAAUX } eSifType;
...
typedef struct
{
	...
	st_zim_Safety_inf		mSafety;
	st_zim_adci_rnginf		iac_rng[DEF_MAX_IAC_RNGCNT];
	st_zim_adcv_rnginf		vac_rng[DEF_MAX_AUX_VACCNT];
	st_zim_vdc_rnginf		vdc_rng[DEF_MAX_AUX_VACCNT];
	st_zim_Eis_Cal_info		mEisIRngCalInfo[MAX_IAC_CTRL_RNGCNT];	
	st_zim_dummy      		mDummy[MAX_IAC_CTRL_RNGCNT];
	double					NoUse[5];
} st_zimaux_rnginf;
...
typedef union st_zim_rnginf_t
{
	st_zimdef_rnginf  main;
	st_zimaux_rnginf  aux;
}st_zim_rnginf, * pst_zim_rnginf;
