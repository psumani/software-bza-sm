...
inline bool ChkSysCalVars(int ch)
{
	...
	ushort mType = m_pSysConfig->mZimCfg[ch].info.cModel[0] - 0x30;
	if(mType == DEV_BZAAUX1)
	{

		if(pRanges->main.rtd_rng.gain == 1.0 || pRanges->main.rtd_rng.offset == 0.0
		   || pRanges->main.vdc_rng[0].gain == 1.0 || pRanges->main.vdc_rng[0].offset == 0.0
			   || pRanges->main.vdc_rng[1].gain == 1.0 || pRanges->main.vdc_rng[1].offset == 0.0)
		{
			return false;
		}
		...
		{
			if(ChkEisCalVar(&pRanges->aux.mEisIRngCalInfo[i]) == false)
			{
				return false;
			}
		}
		...
		{
			if(ChkEisCalGainVar(&pRanges->aux.iac_rng[i]) == false)
			{
				return false;
			}
		}
		for(i=0; i<DEF_MAX_AUX_VACCNT; i++)
		{
			if(ChkEisCalGainVar(&pRanges->aux.vac_rng[i]) == false)
			{
				return false;
			}
			if(pRanges->aux.vdc_rng[i].gain == 1.0 || pRanges->aux.vdc_rng[i].offset == 0.0)
			{
				return false;
			}
		}
	...
	else
	{
		...
		for(i=0; i<MAX_IAC_CTRL_RNGCNT; i++)
		{
			if(ChkEisCalVar(&pRanges->main.mEisIRngCalInfo[i]) == false)
			{
				return false;
			}
		...
		for(i=0; i<DEF_MAX_IAC_RNGCNT; i++)
		{
			if(ChkEisCalGainVar(&pRanges->main.iac_rng[i]) == false)
			{
				return false;
			}
		}
