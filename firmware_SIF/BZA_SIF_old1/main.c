...
int main()
{
	...
	{	
		...
		{
			if(m_pSysConfig->mSIFCfg.Type == (byte)SIF_SBZA || m_pSysConfig->mSIFCfg.Type == (byte)SIF_MBZA || m_pSysConfig->mSIFCfg.Type == (byte)SIF_BZAAUX)
			{
				DeviceMainProc();
			}
		}
