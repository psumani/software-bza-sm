...
int GetMaxBoard(void)
{
	...
	if(m_pSysConfig->mSIFCfg.Type == (byte)SIF_MBZA || m_pSysConfig->mSIFCfg.Type == (byte)SIF_BZAAUX)
	{
		MaxBd = MAX_DEV_BOARD;
	}
...
bool CmdResetProc()	
{
	...
	for(bd=0; bd < m_pGlobalVar->mStatusInf.MaxBoard; bd++)
	{
		...
		if(m_pGlobalVar->mChVar[bd].ResetICE == 1)
		{
			bret =  true;
			break;
		}
		else if(m_pGlobalVar->mChVar[bd].ResetICE == 2)
		{
			if(m_pGlobalVar->mChVar[bd].m_msReset >= 25)
			{
				m_pGlobalVar->mChVar[bd].ResetICE = 0;
			}
			else 
			{
				bret =  true;
		...
		else if(m_pGlobalVar->mChVar[bd].ResetICE == 3)
		{
			if(m_pGlobalVar->mChVar[bd].m_msReset >= 25)
			{
				SetDevBoard(bd);
				Bd_Seldelay();
				Set_IceResetB(false);
				m_pGlobalVar->mChVar[bd].ResetICE = 2;
			}
			bret =  true;
			break;
		}
		...
		{
			if(m_pGlobalVar->mChVar[bd].CmdResetICE == 1)
			{
				m_pGlobalVar->mChVar[bd].ResetICE = 1;
				m_pGlobalVar->mChVar[bd].CmdResetICE = 0;
				m_pGlobalVar->mChVar[bd].m_msReset = 0;
				SetDevBoard(bd);
				Bd_Seldelay();
				Set_IceResetB(true);
				bret =  true;
			...
			{
				m_pGlobalVar->mChVar[bd].ResetICE = 3;
				m_pGlobalVar->mChVar[bd].CmdResetICE = 0;
				m_pGlobalVar->mChVar[bd].m_msReset = 0;
				SetDevBoard(bd);
				Bd_Seldelay();
			...
			else if(m_pGlobalVar->mChVar[bd].CmdResetICE == 3)
			{
				m_pGlobalVar->mChVar[bd].ResetICE = 3;
				m_pGlobalVar->mChVar[bd].CmdResetICE = 0;
				m_pGlobalVar->mChVar[bd].m_msReset = 0;
	...
	for(bd=0; bd < m_pGlobalVar->mStatusInf.MaxBoard; bd++)
	{
		...
		if(m_pSysConfig->ChkZIM[bd] == FALSE)
		{
			...
			if(FindZimProc(bd) == TRUE)
			{
				...
				if(m_pSysConfig->EnaROM[bd] == 0) InitFixRangeInf(bd);
				InitDevice(bd);
...
void DeviceMainProc(void)
{
	...
	if(m_pGlobalVar->m_msAux >= 250) 
	{
		...
		m_pGlobalVar->m_Auxbd ++;
		if(m_pGlobalVar->m_Auxbd >= m_pGlobalVar->mStatusInf.MaxBoard)m_pGlobalVar->m_Auxbd = 0;
	}
	...
	if(m_pGlobalVar->m_msFind > 375) 
	{
		...
		bFindProc = true;
		m_pGlobalVar->m_Findbd ++;
		if(m_pGlobalVar->m_Findbd >= m_pGlobalVar->mStatusInf.MaxBoard)m_pGlobalVar->m_Findbd = 0;
	}
	...
	for(bd=0; bd < m_pGlobalVar->mStatusInf.MaxBoard; bd++)
	{
		...
		if(m_pSysConfig->ChkZIM[bd] == FALSE)
		{
			if(bFindProc == true && m_pGlobalVar->m_Findbd == bd) 
			{
				if(FindZimProc(bd) == TRUE)
				{
				...
				{
					if(m_pGlobalVar->mChVar[bd].m_msReset >= 5000)
					{
						m_pGlobalVar->mChVar[bd].CmdResetICE = 2;
						m_pGlobalVar->mChVar[bd].m_msReset = 0;
						m_pSysConfig->ChkZIM[bd] = FALSE;
						m_pGlobalVar->mChVar[bd].mChStatInf.ErrorStatus = DEF_LAST_ERROR_RESETZIM;
						break;
					}
					m_pGlobalVar->mChVar[bd].mdevice.adc_vdc.adcval = 0x800000;
					m_pGlobalVar->mChVar[bd].mdevice.adc_vdc.value = 0.0;
					m_pGlobalVar->mChVar[bd].mdevice.adc_rtd.data.fault = 1;
					m_pGlobalVar->mChVar[bd].mdevice.adc_rtd.data.adcval = 0x4000;
					m_pGlobalVar->mChVar[bd].mdevice.adc_rtd.data.Tvalue = 0.0;
					m_pGlobalVar->mChVar[bd].mdevice.adc_rtd.data.Rvalue = 100.0;
					m_pGlobalVar->mChVar[bd].mChStatInf.ErrorStatus = DEF_LAST_ERROR_NOZIM;
					m_pSysConfig->ChkZIM[bd] = FALSE;
				}
			}
			continue;
		...
		if(m_pGlobalVar->mChVar[bd].TmpResetICE == 1)
		{
			m_pGlobalVar->mChVar[bd].TmpResetICE = 0;
			m_pGlobalVar->mChVar[bd].CmdResetICE = 2;
			m_pGlobalVar->mChVar[bd].m_msReset = 0;
		...
		if(bAuxProc == true && m_pGlobalVar->m_Auxbd == bd) 
		{
			AuxProc(bd); 
		}
		...
		{
			if(m_pGlobalVar->mChVar[bd].CmdStop == 1)
			{
				if(m_pGlobalVar->mChVar[bd].mChStatInf.LastError == 0)
				{
					proc_stop_test(bd,DEF_LAST_ERROR_MANUALSTOP);
				}
				else
				{
					proc_stop_test(bd,m_pGlobalVar->mChVar[bd].mChStatInf.LastError);
				}
			}
			else
			{
				if(m_pGlobalVar->mChVar[bd].mChStatInf.LastError > DEF_LAST_ERROR_NONE)
				{
					proc_stop_test(bd,m_pGlobalVar->mChVar[bd].mChStatInf.LastError);
				}
				else
		...
		{	
			if(m_pGlobalVar->mChVar[bd].CmdStart == 1)
			{
				proc_start_test(bd);
			}
			else
			{
				if(m_pGlobalVar->mChVar[bd].bTestMode == 0)
				{
					if((m_pGlobalVar->mChVar[bd].flow_dds_sig.req.ctrl & DDS_SIG_PWDN) == 0)
					{
						proc_eis_dcoff(bd);
					}
				}				
