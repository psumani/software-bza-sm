...
{	
	...
	switch(pCmdHeader->Command)
	{
		...
		case DEF_CMD_GET_SYSTEM_INFO:	 
            {
				...
				{
					stSystemConfig m_SysConfig;
					
					m_SysConfig.ID = m_pSysConfig->ID; //
					m_SysConfig.BaseTick = m_pSysConfig->BaseTick;
					m_SysConfig.DaqTick = m_pSysConfig->DaqTick;
					memcpy(&m_SysConfig.mSIFCfg,&m_pSysConfig->mSIFCfg,sizeof(stSIFCfg) + (3 * MAX_DEV_BOARD) + sizeof(stZimCfg));
				
					m_PtrWrData[s] = (byte*)&m_SysConfig;
					m_SendDataSize[s] = sizeof(stSystemConfig);
					m_SendFrameCount[s] = (m_SendDataSize[s] / PACKET_DATA_SIZE) + 1;
				}
				m_SendFrameNo[s] = 0;
		...
		case CMD_SET_MCBZA_RNGINFO :
			{
				if(m_pSysConfig->mSIFCfg.Type == (byte)SIF_MCBZA && pCmdHeader->Slot == 0)
				{
					stSystemConfig sysCfg;
					memcpy(&sysCfg, pCmdData, sizeof(stSystemConfig));
					memcpy(&m_pSysConfig->mZimCfg[0].ranges.Gen, &sysCfg.mZimCfg[0].ranges.Gen, sizeof(st_zimGen_rnginf));
					for(int i=1;i<MAX_DEV_BOARD;i++)
						memcpy(&m_pSysConfig->mZimCfg[i].ranges.Aux, &sysCfg.mZimCfg[i].ranges.Aux, sizeof(st_zimaux_rnginf));
		...
		case CMD_READ_ZIM_ROM:	
			{
				...
				{
					st_zimaux_rnginf   	tranges;
				
					if(ReadRangeInfo(bd, 0,EEPROM_ADDR, (void*)&tranges) == _ERROR)
					{
						SendError(s,DEF_ERROR_NOZIM);
						return;
					}
					m_PtrWrData[s] = (byte*)&tranges;
					m_SendDataSize[s] = sizeof(st_zimaux_rnginf);
				}
				else
				{
					st_zimGen_rnginf   	tranges;
					...
					m_PtrWrData[s] = (byte*)&tranges;
					m_SendDataSize[s] = sizeof(st_zimGen_rnginf);
				}
				m_SendFrameCount[s] = (m_SendDataSize[s] / PACKET_DATA_SIZE) + 1;
				m_SendFrameNo[s] = 0;
				 break;
		...
		case CMD_REFRESH_ZIM_ROM:		
			{
				...
				if(m_pSysConfig->mSIFCfg.Type == (byte)SIF_MCBZA && bd > 0)
				{
					if(ReadRangeInfo(bd, 0,EEPROM_ADDR,&m_pSysConfig->mZimCfg[bd].ranges.Aux) == _ERROR)
					{
						SendError(s,DEF_ERROR_NOZIM);
						return;
					}
				...
				{
					if(ReadRangeInfo(bd, 0,EEPROM_ADDR,&m_pSysConfig->mZimCfg[bd].ranges.Gen) == _ERROR)
					{
						SendError(s,DEF_ERROR_NOZIM);
						return;
					}
	...
	case DEF_LAN_CMD_CALIB_MODE:
		...
		case CMD_GET_RNGINFO : 
			{
				...
				if(m_pSysConfig->mSIFCfg.Type == (byte)SIF_MCBZA && bd > 0)
				{
					m_PtrWrData[s] = (byte*)&m_pSysConfig->mZimCfg[bd].ranges.Aux;
					m_SendDataSize[s] = sizeof(st_zimaux_rnginf);
				}	
				else
				{
					m_PtrWrData[s] = (byte*)&m_pSysConfig->mZimCfg[bd].ranges.Gen;
					m_SendDataSize[s] = sizeof(st_zimGen_rnginf);
				}
				m_SendFrameCount[s] = (m_SendDataSize[s] / PACKET_DATA_SIZE) + 1;
				m_SendFrameNo[s] = 0;
		...
		case CMD_SET_RNGINFO : 
			{
				...
				if(m_pSysConfig->mSIFCfg.Type == (byte)SIF_MCBZA && bd > 0)
				{
					memcpy(&m_pSysConfig->mZimCfg[bd].ranges.Aux, pCmdData, sizeof(st_zimaux_rnginf));
				}
				else
				{
					memcpy(&m_pSysConfig->mZimCfg[bd].ranges.Gen, pCmdData, sizeof(st_zimGen_rnginf));
				}
                break;
            }	
		...
		case CMD_WRITE_ROM :	
			{
				...
				if(pCmdHeader->Slot >= 0 && pCmdHeader->Slot < 4)
				{
					...
					if(m_pSysConfig->EnaROM[bd] == 1)
					{
						...
						if (m_pSysConfig->mSIFCfg.Type == (byte)SIF_MCBZA && bd == 0)
						{
							for (int i = 1; i < MAX_DEV_AUXBOARD+1; i++)
							{
								if(m_pSysConfig->EnaZIM[i] == FALSE || m_pSysConfig->ChkZIM[i] == FALSE)
									continue;
								SetDeviceBoard(i);
								
								if(WriteRangeInfo(i, 0,EEPROM_ADDR) == _ERROR)
								{
									SendError(s,DEF_ERROR_STOREZIMINF);
									return;
								}
		...
		case CMD_GET_STAT_EIS :
			{
				...
				else
				{
					bd = pCmdHeader->Slot;
					if(m_pSysConfig->mSIFCfg.Type == (byte)SIF_MCBZA)
						m_PtrWrData[s] = (byte*)&m_pGlobalVar->mChVar[0].mChStatInf;
					else
				...
				m_SendFrameCount[s] = (m_SendDataSize[s] / PACKET_DATA_SIZE) + 1;
				m_SendFrameNo[s] = 0;
				//unsigned long tmp1 = offsetof(stChStatusInf, Aux_Vdc);
                break;
            }
		...
		case CMD_GET_RAWVAL_EIS :
			{
				...
				m_PtrWrData[s] = (byte*)&m_pGlobalVar->mChVar[bd].meis[ch].eis_raw_new.raw_val[0];
		...
		case CMD_SET_RAWVAL_RELOAD : // tmp
			{
				if(pCmdHeader->Slot < 0 || pCmdHeader->Address < 0)
				{
					SendError(s,DEF_ERROR_CMDFAIL);
