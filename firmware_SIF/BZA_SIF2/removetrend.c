...
Xt�� L���� subsequences �� ������. 
�� �ø����� ��հ� ǥ�������� mu[L], sigma[L]�� ����.
mu�� sigma�� ��ġ�� ������ ���ؼ�, Smu, Ssigma�� ����.
Xt�� stationary�ϴٸ� ��� 0, ǥ������ 1�� ���� ������ �����ؾ� �Ѵ�.

Sampling rate fs�� ���� �����͸� 1�� ���� �о, ��������� ������ ������ Ȯ���Ѵ�.
----------------------------------------------------------------------------------------
*/
...
bool  RemoveTrend(st_zim_eis_raw_val* pXt, st_zim_eis_raw_val* pX, int Ns, int CP,stCompdrift* pCompdrift) 
{
    ...
    pX : output signal, Ns * 1 vector, Memory �Ҵ� �� �Ѱ���� ��.
	Ns : number of data
	CP : Points of a Cycle
	
	X  : detrended output, Ns x 1 vector

	pXt = a + b*t + c*t^2 + A1*sin(w*t + p1)
	   = a + b*t + c*t^2 + d*sin(w*t) + e*cos(w*t)
	where d = A1*cos(p1) and e = A1*sin(p1)
    ...
    p1 = b;
	p2 = c;
    A1 = sqrt(pow(d, 2) + pow(e, 2));
    p1 = atan2(e, d) * (double)180.0 / (double)PI;
	*/
	...
	// sum, a, b, c, d, e, Y, S, newXt : globally defined
	...
	for(i = (MAX_COEF-1); i >= 0; i--)
	{
		...
		if(pR && pR->v && pR->v[i])
		{
			...
			{
				S[i].iac = (Y[i].iac - sum.iac) / pR->v[i][i];   
