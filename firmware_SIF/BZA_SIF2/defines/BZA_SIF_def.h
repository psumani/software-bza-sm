...
#define DEF_AUX_DEVDO_DCSEL		0x01
#define DEF_AUX_DEVDO_ACSEL		0xFE
#define DEF_AUX_DEVDO_VAC_POW_ON		0x02
...
typedef struct 
{
	...
	double 				Aux_Veoc[DEF_MAX_AUX_BDCNT * DEF_MAX_AUX_CHCNT];
	double 				Temperature;
	...
	double 				DispFreq;
	double 				DispMag[DEF_MAX_AUX_BDCNT * DEF_MAX_AUX_CHCNT];
	double 				DispPhase[DEF_MAX_AUX_BDCNT * DEF_MAX_AUX_CHCNT];
	
	ushort				ConnCBL[DEF_MAX_AUX_BDCNT]; // cell cable connected : 1
...
{	
	...
	uint 				m_msRefreshDC;
	uint 				m_msFind;
        uint                            m_msTmp; // when 1. dch ir - eis finished
