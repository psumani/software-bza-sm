...
{
    m_pSysConfig->mSIFCfg.Type = (byte)SIF_MCBZA;
...
void InitConnConfig(void)
{
	...
	sprintf((char*)m_pConnCfg->mEthernetCfg.hostname, "%s%05X", DEF_HOST_NAME,0);
...
void InitAuxDevice(int bd)
{
	...
	memset(&m_pGlobalVar->mChVar[bd].flow_dds_clk,0x0,sizeof(st_zim_dds_flow));
	...
	m_pGlobalVar->mChVar[bd].mreqdevice.dds_clk.frequency = 3200.0; //25600.0; // 1024000.0;
	...
	m_pGlobalVar->mChVar[bd].mdevice.dds_clk.pwdn = 1;
        ...
        m_pGlobalVar->mChVar[bd].flow_dds_clk.req.freq = 25600.0;
	...
	m_pGlobalVar->mChVar[bd].mreqdevice.ctrl_do.data = DEF_AUX_DEVDO_DCSEL;
...
void InitDevice(int bd)
{
	...
	if(m_pSysConfig->mSIFCfg.Type == (byte)SIF_MCBZA)
		m_pGlobalVar->mChVar[bd].mreqdevice.dds_clk.frequency = 3200.0; //25600.0;
	else
		m_pGlobalVar->mChVar[bd].mreqdevice.dds_clk.frequency = 1024000.0;
	m_pGlobalVar->mChVar[bd].mreqdevice.dds_clk.Phase = 0.0;
	m_pGlobalVar->mChVar[bd].mreqdevice.dds_clk.reset = 0;
	m_pGlobalVar->mChVar[bd].mreqdevice.dds_clk.pwdn = 0;
	m_pGlobalVar->mChVar[bd].mreqdevice.dds_clk.Half = 1;
	...
	if(m_pSysConfig->mSIFCfg.Type == (byte)SIF_MCBZA && bd > 0)
	{
		m_pGlobalVar->mChVar[bd].mreqdevice.adc_ac.cfg.vac_flt = DEF_FLT_WIDEBAND1; //
		//m_pGlobalVar->mChVar[bd].mreqdevice.adc_ac.cfg.vac_osr = DEF_OSR_256;
	}
	else
	{
		m_pGlobalVar->mChVar[bd].mreqdevice.adc_ac.cfg.iac_flt = DEF_FLT_WIDEBAND1; 
