...
bool proc_mon_dcControl(int bd)
{
	...
	pdds->Phase = preqdds->Phase = m_pSysConfig->mZimCfg[bd].ranges.Gen.idc_rng.idcofs[pch->mChStatInf.Iac_rngno].offset;
	...
	if(pch->mChStatInf.TaskNo == -1)
	{
		...
		if(bmonitor) 
		{
			...
			for(auxch=0; auxch<DEF_MAX_AUX_CHCNT; auxch ++)
			{
				...
				pch->mChStatInf.eis_status.Aux_zdata[auxch+(bd-1)*4].mag = 0.0;
				pch->mChStatInf.eis_status.Aux_zdata[auxch+(bd-1)*4].phase = 0.0;
			}
...
inline byte FindZimProc(int bd)
{
	...
	if(proc_read_version(bd) == _NO_ERROR)
	{
		...
		{
			if(m_pSysConfig->mZimCfg[bd].info.ZimFWVersion < 5100 && m_pSysConfig->mZimCfg[bd].info.ZimFWVersion >= 8000) // <= 5100?
			{
...
void SetModeAuxBoard(int mode)
{
	...
	for(bd=1; bd < MAX_DEV_AUXBOARD+1; bd++)
	{
		...
		if(mode == 1)
		{
			preqdo->data |= DEF_AUX_DEVDO_DCSEL; //DEF_DEVAUXDO_MODE_DC;
		}

		else
		{
			preqdo->data &= DEF_AUX_DEVDO_ACSEL; //DEF_DEVAUXDO_MODE_AC;
		}
	}
}
...
void procaux_test_main()
{
	...
	if(pch->mChStatInf.TaskNo == -1)
	{
		...
		if(bmonitor) 
		{
			...
			pch->mChStatInf.DispFreq = 0.0;
                        for(int bd=1; bd<=DEF_MAX_AUX_BDCNT; bd++) // added for bd>0 
                        {
...
void CheckAuxBoard(int bd)
{
	...
	if(m_pSysConfig->ChkZIM[bd] == FALSE) 
	{
		...
		if(FindZimProc(bd) == TRUE)
		{
			...
			procaux_power_VAC(bd, false); //proc_power_VAC
			if(m_pSysConfig->EnaROM[bd] == 0) InitFixRangeInf(bd,-1);
			InitAuxDevice(bd);
			m_pSysConfig->ChkZIM[bd] = TRUE;
			pch->TmpResetICE = 0;
