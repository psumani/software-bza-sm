...
inline INT_32 CheckRangeInfo(int ch)
{
	...
	if(isnan(m_pSysConfig->mZimCfg[ch].ranges.rtd_rng.maximum)) return _ERROR;
	if(isnan(m_pSysConfig->mZimCfg[ch].ranges.rtd_rng.minimum)) return _ERROR;
	if(isnan(m_pSysConfig->mZimCfg[ch].ranges.rtd_rng.factor)) return _ERROR;
