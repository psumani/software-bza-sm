...
int16 SendString(SOCKET s, char* pdata)
{
    uint16 len = strlen(pdata);
    char* pTemp = pdata;
    ushort tmax = 1024;    
 
    while(len > 0)
    {
        uint16 tr = len > tmax ? tmax : len;
        
        (void)SOCKET_Send(HTTP_SOCKET, pTemp, tr);
        if (GetStatus(s) != Sn_SR_ESTABLISHED) return 0;
        len -= tr;
        pTemp += tr;
    }
