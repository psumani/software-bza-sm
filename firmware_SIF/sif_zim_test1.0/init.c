...
unsigned char LoadRangeInfo(void)
{
	...
	uint page = NAND_PAGE_RANGEINF;  //1 block = 64(NAND_PG_PER_BLK) page  ... 16 block   17 * 64 = 1024
    bool bchkid = false;
	while(1)
    {
        if(Nand_ReadPage(page, buffer) == FLASH_ERROR)
