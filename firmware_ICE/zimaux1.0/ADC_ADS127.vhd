...
type def_state 				is (s_idle, s_rd_adc0, s_rd_adc1, s_rd_adc2, s_rd_adc3, s_rd_adc4, s_finish, s_rd_adc2_pre, s_rd_adc3_pre);
...
signal drdy_sync1, drdy_sync2, drdy_prev : std_logic := '1';
...
begin
	...
	begin
		if RESET = '1' then
			...
			when s_rd_adc1 =>
				if drdy_falling = '1' then -- DRDY = '0'
					bit_cnt	<= x"00";
					cmd_rdadctmp <= x"00000000";
					adc_state 		<= s_rd_adc3_pre; -- s_rd_adc3;
					CS				<= '0';
				end if;
			...
			when s_rd_adc3 =>
				SCLK 			<= '1';
				bit_cnt 	<= bit_cnt + "1";
				adc_state 	<= s_rd_adc2_pre; -- s_rd_adc2;
